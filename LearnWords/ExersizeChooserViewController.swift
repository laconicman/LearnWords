...
class ExersizeChooserViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        numberOfWordsInSet.text = NSLocalizedString("In set ", comment: "Label total words in current set") + pluralizedWordCount(Storage.wordsAndStat.count) + ". " + NSLocalizedString("Learned ", comment: "Label learned words") + pluralizedWordCount(Storage.wordsAndStat.reduce(0, { result, wAs in
